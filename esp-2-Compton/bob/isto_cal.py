...
elements0= [["cs","$^{137}$Cs", [0.662], [3700,4000], 5e2, "Cs137"],
           ["am","$^{241}$Am", [0.0595], [320,380], 1e4, "Am241"],
           ["na","$^{22}$Na",  [0.511], [2900,3100],5e2, "Na22"],
           ["na","$^{22}$Na",  [1.275], [7100,7400],1e1,"Na22"],
           ["co","$^{60}$Co", [1.173,1.333], [6450,7850],1e2,"Co60"]]
...
for data in date:
    fig = plt.figure('Spettri di calibrazione %s'%(data),figsize=(9, 9), dpi=150)
    fig.set_tight_layout(True)
    fig.clf()
    sub=0
    for b in arange(len(elements0)):
        if b != 4 and data == '20feb': continue
        #if data == '20feb': continue
        ...
        if(nome == "co"):
            ...
            val = popt
            sin = abs(int(popt[1]-1.8*abs(popt[2])))
        ...
        if(energy!=elements[3][2]):
            sub = sub+1
            temp = fig.add_subplot(2,2,sub)
            #figure(data+" "+el+"_spectrum",figsize=(9, 4), dpi=150).set_tight_layout(True)
            title(el)
            temp.grid()
            #temp.minorticks_on()
            temp.set_xlabel("energia [digit]")
            temp.set_ylabel("conteggi")
            k=8
            n_bin = len(Y)//k
            ...
            bar_line(X_rebin, Y_rebin, linewidth=1, color=[0.6]*3,label="dati")
    
            if(nome=="co"):
                temp.plot(z,gaus2(z,*popt),color=[0]*3,linewidth=1,label="fit")
                #plot(z,fondo(z,popt[6],popt[7]))
            else:
                temp.plot(z,distr(z,*popt),color=[0]*3,linewidth=1,label="fit")
                #plot(z,fondo(z,popt[3],popt[4]), color="blue")
        else:
            temp.plot(z,distr(z,*popt),color=[0]*3,linewidth=1)
        temp.legend()
...
#        savefig("cal/plot/"+data+"_"+el+"_"+str(energy[0])+".png")
    ...
    #    print("")
        ...
        if(nome=="co"):
            print("%s \t %s \t %f \t %f \t %f \t %f \t %f \t %f \t %f" %(data, a[5], energy[0],popt[1],abs(popt[2]),sqrt(pcov[1][1]),sqrt(pcov[2][2]),pcov[1][2], pcov[1,4]),file=file_print)
            print("%s \t %s \t %f \t %f \t %f \t %f \t %f \t %f \t %f" %(data, a[5], energy[1],popt[4],abs(popt[5]),sqrt(pcov[4][4]),sqrt(pcov[5][5]),pcov[4][5], 0),file=file_print)
        else:
            print("%s \t %s \t %f \t %f \t %f \t %f \t %f \t %f \t %f" %(data, a[5], energy[0],popt[1],abs(popt[2]),sqrt(pcov[1][1]),sqrt(pcov[2][2]),pcov[1][2], 0),file=file_print)
