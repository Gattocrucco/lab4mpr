...
\end{itemize}
 ...
 \emph{Petrillo}}
  \marginpar{Ma non era $c=\hslash=1$? (Andrea)}
 \begin{equation}
...
\marginpar{Proporrei (e quindi l'efficienza di rivelazione) in modo che non si possa fraintendere \\ (Andrea)}
 ...
 Un rivelatore di radiazione ideale dovrebbe essere lineare. Come già detto la funzione di risposta del nostro spettrometro è la composizione delle funzioni di risposta dei singoli processi che la compongono.
 \begin{itemize}
 	...
 	\item 
 \end{itemize}
 \'E noto che la funzione di risposta 
 
  In the case of NaI(Tl) detectors, it was recognized early that the
 scintillation light is not proportional to the deposited energy. Fig. 5.20 shows the relative light
 yield for fully stopped electrons as a function of the electron energy.
 
\marginpar{Coooosaaaaaaaaaaa??}
 
 \paragraph{Distribuzione di risposta (risoluzione)}
 
 \paragraph{Calibrazione} %aggiungi nota sull'americio
