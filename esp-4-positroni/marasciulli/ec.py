...
H, _, _= np.histogram2d(ch1_c2, ch2_c2, bins=bins)

ax_colorbar.axis('off')

ax_c2_ch1.set_xlabel('energia PMT 1  [keV]')
ax_c2_ch2.set_ylabel('energia PMT 2  [keV]')

H1, _, _ = ax_ch1.hist(ch1, bins=bins, histtype='step', log=True)
H2, _, _ = ax_ch2.hist(ch2, bins=bins, histtype='step', log=True)

ax_ch1.set_xlabel('energia PMT 1  [keV]')
ax_ch2.set_xlabel('energia PMT 2  [keV]')
...
for key in cuts.keys():
    ...
    outputs, inputs = fit_peak.fit_peak_2d(bins, bins, hist, cut=cut, bkg=bkgs[key], print_info=0, plot_cut=True, corr=key == 'betabeta')
    norm[key] = outputs['norm'] / (bins[1] - bins[0]) ** 2 * rate_corr
    mean[key]=outputs['mean']
...
b1=gvar.mean(mean['betabeta'][0])
n1=gvar.mean(mean['gammabeta'][0])
b2=gvar.mean(mean['betabeta'][1])
n2=gvar.mean(mean['betagamma'][1])

q1=(b1*ne-n1*me)/(b1-n1)
m1=(me-q1)/b1

q2=(b2*ne-n2*me)/(b2-n2)
m2=(me-q2)/b2
...
ax_c2_ch1.hist(ch1_e, bins=bins*m1+q1, orientation='vertical', histtype='step', log=True)
ax_c2_ch2.hist(ch2_e, bins=bins*m2+q2, orientation='horizontal', histtype='step', log=True)

E, _, _, im = ax_c2.hist2d(ch1_e, ch2_e, bins=[bins*m1+q1,bins*m2+q2], norm=colors.LogNorm(), cmap='jet')
fig_c2.colorbar(im, ax=ax_colorbar, fraction=0.5, aspect=2)

# aggiunta delle legende belle    

ax_c2_ch1.text((gvar.mean(mean['betabeta'][0])-20)*m1+q1,50,'Na$_{\\beta}$')
ax_c2_ch2.text(50,(gvar.mean(mean['betabeta'][1])-5)*m2+q2,'Na$_{\\beta}$')

ax_c2_ch1.text((gvar.mean(mean['gammabeta'][0])-5)*m1+q1,8,'Na$_{\\gamma}$')
ax_c2_ch2.text(8,(gvar.mean(mean['betagamma'][1])-5)*m2+q2,'Na$_{\\gamma}$')

ax_c2_ch1.text((gvar.mean(mean['gammabetabeta'][0])-100)*m1+q1,1,'Na$_{\\beta}$+Na$_{\\gamma}$')
ax_c2_ch2.text(0.5,(gvar.mean(mean['betagammabeta'][1])-10)*m2+q2,'Na$_{\\beta}$+Na$_{\\gamma}$')
...
for key in cuts:
    ...
    kw = dict(cut=cut, bkg='exp', print_info=0, ax=[ax_ch1, ax_ch2][key[1] - 1], plot_kw=dict(scaley=False))
    if key[0] == 'beta':
